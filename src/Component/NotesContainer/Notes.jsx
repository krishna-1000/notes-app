...
const Notes = () => {
  ...
  const {duration,setDuration,timerCountDown} = useCountDown();
  const {minutes,seconds} = timerCountDown(duration);
  return (
    <div className='border-l-1 border-r-1 border-gray-500 ml-15 mr-15 h-full'>
      <textarea value={notes} onChange={(e) => { addNote(e.target.value) }} placeholder='Write your notes here...' className='w-[90vw] p-2 h-[90vh] text-xl overflow-auto scrollbar   outline-none mt-2 resize-none'>
      </textarea>
